...
class Ui_mesbox(QWidget):
    def __init__(self, text):
        ...
        self.t = None
        p = QtGui.QPalette()
        p.setBrush(self.backgroundRole(), QtGui.QBrush(QtGui.QPixmap("resource/图片/pix.png")))
        self.setPalette(p)
        ...
        winsound.PlaySound("resource/字体和音乐/Windows Background.wav", 1)
        self.settmd()
...
class TableItem(QTableWidgetItem):
    def __init__(self, text):
        ...
        if text == '播放':
            self.setIcon(QtGui.QIcon("resource/图片/play1.png"))
        else:
            if text == '下载':
                self.setIcon(QtGui.QIcon("resource/图片/down1.png"))
            else:
                if text == '删除':
                    self.setIcon(QtGui.QIcon("resource/图片/delete1.png"))
                else:
                    if text == '移动':
                        self.setIcon(QtGui.QIcon("resource/图片/move1.png"))
                    else:
                        if text == '添加至歌单':
                            self.setIcon(QtGui.QIcon("resource/图片/add1.png"))
                        else:
                            self.setText(text)
    ...
    def __init__(self, parent, type):
        ...
        if type > 0:
            self.setStyleSheet(
                ...
                "QSlider::handle:vertical { border-image: url(resource/图片/vo.png); margin: -2px -7px -2px "
                "-7px; height: 17px; }")
        else:
            self.setStyleSheet(
...
class TableWidget(QTableWidget):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("QTableWidget::Item:hover{background-color:rgb(74, 136, 199) "
                           ...
                           "179);}QTableWidget{outline:0px;border-image:url(resource/图片/bg.png);color:%s}QScrollBar{"
                           ...
                           "background:transparent;}QHeaderView::section{border:2px groove gray; "
                           "padding:2px 4px; background:transparent; color:black;font-weight:600;width:20px }" % FC)
...
class ListWidget(QListWidget):
    def __init__(self, *args):
        ...
        self.setStyleSheet("QListWidget{border-image:url(resource/图片/bg.png);color:%s;font-size:16px}" % FC)
...
class player(QObject):
    ...
    def __init__(self, w, lrc_map, pgar):
        ...
        path_lrc_map = "resource/文件/lrc.html"
    ...
    def myplay(self):
        ...
        self.toplrc.playing = True
        self.toplrc.tableWidget.item(0, 1).setIcon(QtGui.QIcon('resource/图片/pause1.png'))
    ...
    def stop(self):
        ...
        self.pgar.setFormat('00:00')
        self.changepix.emit(-1)
        self.toplrc.playing = False
        self.toplrc.tableWidget.item(0, 1).setIcon(QtGui.QIcon('resource/图片/play1.png'))
...
class Ui_Widget(QWidget):
    ...
    def setupUi(self):
        ...
        self.jmpath = Label(self.frame_9)
        self.jmpath.settext('解码程序:')
        self.jmpath.setObjectName("jmpath")
        self.horizontalLayout_14.addWidget(self.jmpath)
        self.jmpathedit = QLineEdit(self.frame_9)
        self.jmpathedit.setObjectName("jmpathedit")
        self.horizontalLayout_14.addWidget(self.jmpathedit)
        self.setjmpath = ToolBttun(self.frame_9, 32, 32)
        self.setjmpath.setObjectName("setjmpath")
        self.horizontalLayout_14.addWidget(self.setjmpath)
    ...
    def mysetupUi(self):
        ...
        self.setText(self.downpathedit,gm.st.conf.get('path_section', 'path'))
        self.setText(self.jmpathedit,gm.st.conf.get('path_section', 'jmexe'))
        self.setText(self.pathedit,gm.st.conf.get('path_section', 'path_exe'))
        self.settmd.setValue(int(int(gm.st.conf.get('lrc_section', 'fc1').split(',')[3]) / 255 * 100))
        self.setpath.setText('…')
        self.setjmpath.setText('…')
        self.downpathedit.setEnabled(False)
        self.jmpathedit.setEnabled(False)
        ...
        self.previous.setStyleSheet("QToolButton{border-image:url(resource/图片/Previous.png)}")
        self.set.setStyleSheet("border-image:url(resource/图片/set1.png)")
        self.set.setToolTip('设置')
        self.show_player_list.setStyleSheet("QToolButton{border-image:url(resource/图片/play_list.png)}")
        ...
        self.playbutton.setIcon(QtGui.QIcon('resource/图片/play.png'))
        self.bjpix.setChecked(gm.st.conf.getboolean('base', 'isauto'))
        setPix(self, "resource/图片/pix.png", gm.st.conf.getboolean('base', 'isauto'))
        ...
        if len(gm.st.conf.get('path_section', 'jmexe')) == 0:
            self.printt('正在初始化应用程序……', 60000*3)
            t = threading.Thread(target=self.ini)
            t.setDaemon(True)
            t.start()
    ...
    def setText(self,lineedit:QLineEdit,text:str):
        lineedit.setText(text)
        lineedit.setToolTip(text)

    # 初始化寻找解码程序
    def ini(self):
        l = fs.search_one('ffmpeg.exe')
        for x in l:
            if 'resource/文件/ffmpeg.exe' in x:
                gm.st.set_confi('path_section', 'jmexe', x)
                continue
            p = fs.subprocess.Popen("%s -h" % x, shell=True)
            p.wait()
            if p.returncode == 0:
                gm.st.set_confi('path_section', 'jmexe', x)
                if fs.delete_file('resource/文件/ffmpeg.exe'):
                    self.printt('解码应用程序获得，原始解码程序删除', 5000)
                else:
                    self.printt('解码应用程序获得', 5000)
                return
        self.printt('解码应用程序初始化完毕', 5000)
    ...
    def add_gd(self, gd_name, tableWidet: QTableWidget, index):
        try:
            ...
            self.printt(gd_name+" 添加成功")
        except Exception as e:
            self.printt(gd_name+" 添加失败")
            self.printt(str(e))
    ...
    def changepix(self, type):
        if type > 0:  # 暂停
            ...
            self.playbutton.setIcon(QtGui.QIcon('resource/图片/Pause.png'))
    ...
    def show_song_list(self):
        try:
            ...
            if len(self.player.gd) > 0:
                self.set_table_item(self.gdsong_list, fs.read_json(self.player.gd[0], True), '歌单')
            self.gd_list.addItem(QListWidgetItem(QtGui.QIcon('resource/图片/add1.png'), '添加歌单'))
    ...
    def mouseReleaseEvent(self, event: QtGui.QMouseEvent):
        if self.mouse_left:
            ...
            if (cha.x() > 100 and abs(cha.y()) < 35) or (cha.x() < -100 and abs(cha.y()) < 35):
                ...
                if i == 1:
                    self.stackedWidget.setCurrentIndex(0)
            if self.stackedWidget.currentIndex() == 0 and abs(cha.x()) < 35 and cha.y() > 80:
                if self.type_seach.currentIndex() < 4:
    ...
    def closeEvent(self, a0: QtGui.QCloseEvent) -> None:
        if self.pop is not None and not self.pop.isHidden():
            winsound.PlaySound("resource/字体和音乐/mesbox.wav", 1)
            a0.ignore()
        else:
            if fs.os.path.exists(gm.st.path_json + '/play_list.json'):
    ...
    def setppath(self):
        ...
        if dirname=='':
            return
        self.setText(self.pathedit, dirname + '/Yuye')
    ...
    def setdownppath(self):
        ...
        if dirname=='':
            return
        self.setText(self.downpathedit, dirname)
        gm.st.set_confi('path_section', 'path', dirname)
        gm.st.path = dirname
        self.printt('更换下载路径成功')

    # 重新设置解码路径
    def setjmppath(self):
        filename = QFileDialog().getOpenFileName(filter='*.exe')
        if filename[0] == '':
            return
        self.setText(self.jmpathedit, filename[0])
        gm.st.set_confi('path_section', 'jmexe', filename[0])
        self.printt('手动设置解码程序成功')
    ...
    # 改变背景图片
    def updatepix(self):
        dirname = QFileDialog().getOpenFileName(filter='*.jpg;;*.png;;*JPEG;;All Files(*)')
        fs.shutil.copyfile(dirname[0], 'resource/图片/pix.png')
        setPix(self, "resource/图片/pix.png", self.bjpix.isChecked())
        self.pix_show.setPixmap(QtGui.QPixmap("resource/图片/pix.png"))
    ...
    def savefilter1(self):
        ...
        self.printt('添加类型:'+self.addfilter.text())
        self.addfilter.clear()

    def savefilter2(self):
        self.printt('移除类型:'+self.filterBox.currentText())
    ...
    def setslots(self):
        ...
        self.setjmpath.clicked.connect(self.setjmppath)
